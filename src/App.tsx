...
import { DirectoryPickerGlobal } from "@/components/settings/DirectoryPickerDialog";
...
function App() {
  // Block browser access — only native-shell or dev mode allowed
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-background text-foreground p-8">
        <div className="text-center max-w-md">
          <h1 className="text-2xl font-bold mb-4">CC Switch</h1>
          <p className="text-muted-foreground mb-2">
            CC Switch 只能通过桌面应用程序打开。
          </p>
          <p className="text-sm text-muted-foreground">
            Please use the CC Switch desktop application.
            Browser access is not supported.
          </p>
        </div>
      </div>
    );
  ...
  const handleDisable = useCallback(
    async (provider: Provider) => {
      try {
        ...
        toast.success(t("provider.disabled", { defaultValue: "已停用" }));
      } catch (error) {
        const detail = extractErrorMessage(error) || t("common.unknown");
        toast.error(
          t("provider.disableFailed", {
            defaultValue: "停用失败：{{error}}",
            error: detail,
          }),
          { duration: 6000 },
        );
