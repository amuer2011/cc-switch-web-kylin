...
export function ProviderActions({
  ...
  const handleMainButtonClick = () => {
    ...
    } else if (isCurrent && onDisable) {
      // 独占模式（Claude/Codex/Gemini）：停用当前供应商
      onDisable();
    } else {
      onSwitch();
    }
    ...
    if (isCurrent) {
      if (onDisable) {
        return {
          ...
          variant: "secondary" as const,
          className:
            "bg-gray-200 text-muted-foreground hover:bg-red-100 hover:text-red-600 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-red-900/50 dark:hover:text-red-400",
          icon: <Square className="h-4 w-4" />,
          text: t("provider.disable", { defaultValue: "停用" }),
        };
      }
      return {
        disabled: true,
