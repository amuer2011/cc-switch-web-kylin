...
export function ProxyTabContent({
  ...
  // 乐观状态：在用户点击后立即模拟开关状态，直到后端确认
  const [optimisticRunning, setOptimisticRunning] = useState<boolean | null>(null);
  const displayRunning = optimisticRunning ?? isRunning;

  // 当实际运行状态与乐观状态匹配时，解除乐观强制
  useEffect(() => {
    if (optimisticRunning !== null && isRunning === optimisticRunning) {
      setOptimisticRunning(null);
    }
  }, [isRunning, optimisticRunning]);

  const handleToggleProxy = useCallback(async (checked: boolean) => {
    try {
      if (!checked) {
        setOptimisticRunning(false);
        await stopWithRestore();
      } else if (!settings?.proxyConfirmed) {
        // 未确认过，显示确认对话框（不设置乐观状态，避免闪烁）
      ...
      setOptimisticRunning(null); // 出错时立即回滚
    }
  }, [settings?.proxyConfirmed, stopWithRestore, startProxyServer]);

  const handleProxyConfirm = useCallback(async () => {
        ...
        >
          <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-muted/50 data-[state=open]:bg-muted/50">
            <div className="flex items-center gap-3">
              ...
              <Badge
                variant={displayRunning ? "default" : "secondary"}
                className="gap-1.5 h-6 ml-auto mr-2"
              >
                <Activity
                  className={`h-3 w-3 ${displayRunning ? "animate-pulse" : ""}`}
                />
                {displayRunning
          ...
          <AccordionContent className="px-6 pb-6 pt-4 border-t border-border/50">
            <ProxyPanel
              ...
              forceRunning={optimisticRunning ?? undefined}
            />
          </AccordionContent>
        </AccordionItem>
