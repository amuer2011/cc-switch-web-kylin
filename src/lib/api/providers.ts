...
export const providersApi = {
  ...
  /**
   * Disable the current provider without activating another.
   * Exclusive-mode apps (Claude/Codex/Gemini): clears the active provider status
   * and removes the live config. Additive-mode apps: removes from live config.
   * OMO: disables the current OMO provider.
   */
  async disable(id: string, appId: AppId): Promise<boolean> {
    return await invoke("disable_provider", { id, app: appId });
  },
