...
export const useDisableProviderMutation = (appId: AppId) => {
  const queryClient = useQueryClient();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: async (providerId: string): Promise<boolean> => {
      return await providersApi.disable(providerId, appId);
    },
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["providers", appId] });
      ...
      try {
        ...
        console.error(
          "Failed to update tray menu after disabling provider",
          trayError,
        );
      }
    },
    onError: (error: Error) => {
      const detail = extractErrorMessage(error) || t("common.unknown");
      toast.error(
        t("notifications.disableFailedTitle", { defaultValue: "停用失败" }),
        {
          description: t("notifications.disableFailed", {
