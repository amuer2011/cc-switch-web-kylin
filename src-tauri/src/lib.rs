...
pub use database::{Database, ModelPricingRecord};
...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化全局出站代理 HTTP 客户端
            initialize_global_proxy(&app_state);
            ...
            tauri::async_runtime::spawn(async move {
                ...
                initialize_common_config_snippets_non_tauri(&state);
...
// ============================================================
// Shared initialization logic (used by both Tauri and Web modes)
// ============================================================

/// 初始化数据库默认数据：live 配置导入、官方预设、MCP、Prompts 等
pub fn initialize_db(state: &AppState) {
    // 1. 初始化默认 Skills 仓库
    ...
    // 1.1. Skills 统一管理迁移 / 自动导入
    // 如果 installed_skills 表为空，则扫描各应用目录（Claude、Codex 等）中已安装的 skill 并导入。
    // 这同时处理两种场景：
    //   - 旧版升级（skills_ssot_migration_pending 标记）：使用快照信息导入
    //   - 全新安装（无标记）：直接扫描文件系统导入
    let has_installed_skills = state.db
        .get_all_installed_skills()
        .map(|skills| !skills.is_empty())
        .unwrap_or(false);

    if has_installed_skills {
        // 已有数据，仅清理残留的迁移标记
        if let Ok(Some(flag)) = state.db.get_setting("skills_ssot_migration_pending") {
            if flag == "true" || flag == "1" {
                log::info!("Detected skills_ssot_migration_pending but skills table not empty; skipping auto import.");
                let _ = state.db.set_setting("skills_ssot_migration_pending", "false");
            }
        }
    } else {
        match migrate_skills_to_ssot(&state.db) {
            Ok(count) => {
            ...
            Err(e) => {
                log::warn!("✗ Failed to auto import skills to SSOT: {e}");
    ...
    let first_run_already_confirmed = settings::get_settings()
        .first_run_notice_confirmed
        .unwrap_or(false);
    let fresh_install_at_startup = state.db.is_providers_empty().unwrap_or(false);

    for app_type in app_config::AppType::all().filter(|t| !t.is_additive_mode()) {
        match ProviderService::import_default_config(state, app_type.clone()) {
            Ok(true) => log::info!("✓ Imported live config for {} as default provider", app_type.as_str()),
            Ok(false) => log::debug!("○ {} already has providers; live import skipped", app_type.as_str()),
    ...
    if state.db.is_mcp_table_empty().unwrap_or(false) {
        ...
        for import_fn in [
            ...
            match import_fn(state) {
                Ok(count) if count > 0 => log::info!("✓ Imported {count} MCP server(s)"),
                Ok(_) => log::debug!("○ No MCP servers found to import"),
                Err(e) => log::warn!("✗ Failed to import MCP: {e}"),
            }
        }
    }
    ...
    if state.db.is_prompts_table_empty().unwrap_or(false) {
        ...
        for app in [
            ...
            app_config::AppType::OpenCode,
            app_config::AppType::OpenClaw,
            app_config::AppType::Hermes,
        ] {
            match PromptService::import_from_file_on_first_launch(state, app.clone()) {
                Ok(count) if count > 0 => log::info!("✓ Imported {count} prompt(s) for {}", app.as_str()),
...
fn config_omo_import(state: &AppState, variant: &str) -> Result<crate::Provider, AppError> {
    use crate::services::omo::{OmoService, STANDARD, SLIM};
    let config = if variant == "omo-slim" { &SLIM } else { &STANDARD };
    OmoService::import_from_local(state, config)
}

fn config_omo_slim_import(state: &AppState) -> Result<crate::Provider, AppError> {
    config_omo_import(state, "omo-slim")
}

/// 初始化全局出站代理 HTTP 客户端
pub fn initialize_global_proxy(state: &AppState) {
    let db = &state.db;
    ...
    if let Err(e) = crate::proxy::http_client::init(proxy_url.as_deref()) {
        ...
        if let Err(fallback_err) = crate::proxy::http_client::init(None) {
            log::error!("[GlobalProxy] [GP-008] Failed to initialize direct connection: {fallback_err}");
        }
    }
}
...
pub async fn restore_proxy_state_on_startup(state: &store::AppState) {
...
pub fn initialize_common_config_snippets_non_tauri(state: &store::AppState) {
    for app_type in app_config::AppType::all() {
        if !state
            .db
        ...
        match ProviderService::extract_common_config_snippet_from_settings(
            ...
            Ok(_) => log::debug!("○ Live config for {} has no extractable common fields", app_type.as_str()),
    ...
    if should_run_legacy_migration {
        for app_type in [
            ...
            if let Err(e) = ProviderService::migrate_legacy_common_config_usage_if_needed(state, app_type.clone()) {
