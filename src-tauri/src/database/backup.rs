...
impl Database {
    ...
    pub(crate) fn import_sql_string_for_sync(&self, sql_raw: &str) -> Result<String, AppError> {
        // Newer CC Switch releases may export additive schema versions that
        // this compatibility build does not know how to migrate yet. The SQL
        // snapshot already contains the complete table definitions, so keep
        // the import usable by treating the imported schema as the current
        // local baseline instead of rejecting it solely on user_version.
        let normalized = normalize_sync_sql_user_version(sql_raw);
        self.import_sql_string_inner(&normalized, SYNC_PRESERVE_TABLES)
    }
...
/// Rebase a newer sync snapshot's SQLite user_version to the local schema
/// baseline. This only affects the transient import SQL; the remote snapshot
/// and the local database version are not modified directly.
fn normalize_sync_sql_user_version(sql: &str) -> String {
    ...
    for line in sql.lines() {
        ...
        if lower.starts_with("pragma user_version=") {
            let prefix_len = trimmed.find('=').map(|index| index + 1).unwrap_or(0);
            let value = trimmed[prefix_len..]
                .trim_end_matches(';')
                .trim()
                .parse::<i32>();
            if value.is_ok_and(|version| version > super::SCHEMA_VERSION) {
                let indent_len = line.len() - trimmed.len();
                output.push_str(&line[..indent_len]);
                output.push_str(&format!("PRAGMA user_version={};", super::SCHEMA_VERSION));
                changed = true;
            } else {
...
mod tests {
    ...
    #[test]
    fn sync_import_accepts_newer_schema_user_version() -> Result<(), AppError> {
        let remote_db = Database::memory()?;
        {
            let conn = crate::database::lock_conn!(remote_db.conn);
            conn.execute(
                "INSERT INTO providers (id, app_type, name, settings_config, meta)
                 VALUES ('remote-newer-schema', 'claude', 'Remote Provider', '{}', '{}')",
                [],
            )?;
        }
        let remote_sql = remote_db
            .export_sql_string_for_sync()?
            .replace("PRAGMA user_version=0;", "PRAGMA user_version=16;");
        ...
        let imported: i64 = {
            let conn = crate::database::lock_conn!(local_db.conn);
            conn.query_row(
                "SELECT COUNT(*) FROM providers WHERE id = 'remote-newer-schema'",
                [],
                |row| row.get(0),
            )?
        };
