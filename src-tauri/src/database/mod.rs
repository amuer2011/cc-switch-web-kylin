...
impl Database {
    ...
    pub fn get_model_pricing_records(&self) -> Result<Vec<ModelPricingRecord>, AppError> {
    ...
    pub fn update_model_pricing_record(&self, record: &ModelPricingRecord) -> Result<(), AppError> {
        let conn = lock_conn!(self.conn);
        conn.execute(
    ...
    pub fn delete_model_pricing_record(&self, model_id: &str) -> Result<(), AppError> {
        let conn = lock_conn!(self.conn);
