...
/// Remove the live configuration for an exclusive-mode app (Claude / Codex / Gemini).
///
/// Deletes the live config file(s) so the CLI tool has no active provider configuration.
pub(crate) fn remove_live_config(app_type: &AppType) -> Result<(), AppError> {
    match app_type {
        AppType::Claude => {
            let path = get_claude_settings_path();
            if path.exists() {
        ...
        // Additive-mode apps use per-provider removal, not bulk config deletion
        _ => {}
    }
    Ok(())
}
