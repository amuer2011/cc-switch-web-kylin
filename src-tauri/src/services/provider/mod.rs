...
pub(crate) use live::{
    ...
    provider_exists_in_live_config, remove_live_config, strip_common_config_from_live_settings,
...
impl ProviderService {
    ...
    /// Disable a provider — clear its "current" status without activating another.
    ///
    /// For exclusive-mode apps (Claude/Codex/Gemini): clears `is_current` in DB,
    /// clears the local settings entry, and removes the live config so the app
    /// has no active provider. The provider record itself is preserved.
    ///
    /// For OMO providers: delegates to the existing OMO disable path.
    /// For additive-mode apps: delegates to `remove_from_live_config`.
    pub fn disable(state: &AppState, app_type: AppType, id: &str) -> Result<(), AppError> {
        ...
        if matches!(app_type, AppType::OpenCode) {
            match provider.category.as_deref() {
                ...
                _ => {} // non-OMO OpenCode falls through to additive path below
            }
        }

        // ── Additive-mode path ──
        if app_type.is_additive_mode() {
            return Self::remove_from_live_config(state, app_type, id);
        }

        // ── Exclusive-mode path (Claude / Codex / Gemini) ──
        // Clear local settings (device-level)
        crate::settings::set_current_provider(&app_type, None)?;
        // Clear DB is_current
        state.db.clear_current_provider(app_type.as_str())?;
        // Remove the live config so the app has no active provider
        remove_live_config(&app_type)?;

        // Update live_config_managed flag
        if let Some(mut provider) = state.db.get_provider_by_id(id, app_type.as_str())? {
            Self::set_provider_live_config_managed(&mut provider, false);
            state.db.save_provider(app_type.as_str(), &provider)?;
        }
